namespace GPConnect.PatientFacingService.Provider.AcceptanceTests.Steps
{
	...
	using System;
	using System.Linq;
	using GPConnect.PatientFacingService.Provider.AcceptanceTests.Enum;
	...
	public sealed class StructuredUncategorisedSteps : BaseSteps
	{
        ...
        [Given(@"I add the uncategorised data parameter end date after currentDate")]
        public void GivenIAddTheUncategorisedDataParameterEndDateAftercurrentDate()
        {
            var backDate = DateTime.UtcNow.AddDays(-10);
            var futureDate = DateTime.UtcNow.AddDays(+15);
            var startDate = backDate.ToString("yyyy-MM-dd");
